...
public class UI_MainMenu : BaseUIScript
{
    ...
    public bool CheckSaveState()
    {
        ...
        else
        {
            ...
            Button tmp = menuButtons.Find(x => x.gameObject.name == "Start btn");
            if(GameManager.instance.GetCurrentLevel() == 51)
            {
                tmp.onClick.AddListener(delegate {
                    GameManager.instance.GetLevelManager().LoadSceneByIndex(50);
                    GameManager.instance.SetGamemode(0);
                });
            }
            else
            {
                tmp.onClick.AddListener(delegate {
                    GameManager.instance.GetLevelManager().LoadSceneByIndex(GameManager.instance.GetCurrentLevel());
                    GameManager.instance.SetGamemode(0);
                });
            }          
            CheckIfLevelSelectOpen(menuButtons.Find(x => x.name == "Level Select btn"));
